...
export default function CliperraSignUp() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const click_id =
      typeof window !== "undefined"
        ...
        : null;
    const res = await fetch("/api/cliperra/track-lead", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, click_id }),
    });
    if (res.ok) {
      window.location.href = "/cliperra/dashboard";
    } else {
      setSubmitting(false);
  ...
  return (
    <main className="min-h-screen bg-white text-neutral-900">
      ...
      <section className="container mx-auto max-w-md px-6 py-20">
        <h1 className="text-3xl font-semibold tracking-tight">Start the 7-day trial</h1>
        <p className="mt-3 text-neutral-600">
          No card needed to start. Cancel inside the app any time during the trial.
        </p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          >
            {submitting ? "Creating account…" : "Edit your first clip"}
          </button>
        </form>
      </section>
    </main>
